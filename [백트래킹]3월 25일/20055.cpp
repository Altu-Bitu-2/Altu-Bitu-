...
void second(int n, int start, int end) {
	...
	while (cur != start) { //end - 1 부터 start 까지 검사
		...
		int next = (cur + 1) % (n * 2); //다음 위치, 인덱스가 2n이상인경우 때문에 나머지 연산 수행
...
void third(int start) {
	if (belt[start].power) { //올리는 위치의 내구도가 0이 아니라면 로봇 올림
		...
		belt[start].power--; //로봇을 올리는 위치의 칸의 내구도 1 감소
...
int main() {
	...
	//입력
	cin >> n >> k;
	belt.assign(n * 2, { 0, false }); //내구도 0, 로봇 없음으로 초기화
