...
ci cntStrikeBall(string &s1, string &s2) {
	...
	return ci(strike_cnt, ball_cnt); //스트라이크, 볼 수를 객체로 바꿈
}

int game(int n, vector<baseball> &question) {
	int ans = 0; //정답 개수

	for (int i = 123; i <= 987; i++) { //123부터 987까지 순차 증가
		bool check = true;
...
int main() {
	...
	for (int i = 0; i < n; i++) { //0부터 n까지 순차 증가
		cin >> question[i].num >> question[i].strike >> question[i].ball; //질문한 숫자, 스트라이크 수, 볼 수
	}
